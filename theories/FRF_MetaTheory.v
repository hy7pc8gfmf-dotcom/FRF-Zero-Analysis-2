...
(* ======================== 4. 证明前置（无逻辑断层，依赖Coq标准库引理，无Mathlib残留） ======================== *)
(* ### 4.1 功能特征与权重引理（无逻辑变更，仅依赖Coq标准库） *)
(* 4.1.1 核心功能无重复→互不相同（依赖Coq.Lists.ListDec的NoDup_impl_NoDupA） *)
...
(* 4.1.2 边缘功能权重合规→单个权重≤1（依赖Coq.Reals的 lia 战术） *)
...
(* ======================== 5. 核心定理（形式化完备，无逻辑变更，仅依赖来源更新） ======================== *)
(* ### 5.1 功能角色决定身份（FRF核心，无变更） *)
...
(* ======================== 6. 模块导出（关键：显式导出`carrier`相关，解决ChurchZero.v引用错误） ======================== *)
