...
Record FunctionalRole (S : FormalSystem) : Type := {
  ...
  core_no_dup : list (list string) -> Prop;  (* 核心功能无重复 *)
  edge_no_dup : list string -> Prop;         (* 边缘功能无重复 *)
  core_edge_disjoint : Prop;                (* 核心/边缘功能无交集 *)
  edge_weight_valid : forall (f : string * R), In f edge_features -> snd f ∈ [0, 1]; (* 权重合法 *)
  edge_weight_normalized : sum (fun f => snd f) edge_features ≤ 1; (* 权重归一化 *)
}.
...
Record ConceptIdentity (S : FormalSystem) (obj : S.(carrier)) : Type := {
  ...
  ci_unique : forall (obj' : S.(carrier)) (r' : FunctionalRole S) (rels' : list (DefinitiveRelation S)),
    core_function r' obj' = core_function ci_role obj ∧
...
(* 7. 辅助函数：获取对象的概念身份（无循环依赖，对接基础库实例） *)
Definition cid_of (S : FormalSystem) (obj : S.(carrier)) : ConceptIdentity S obj :=
  ...
  | "Algebra_Monoid" => 
      let role := {|
        ...
        core_no_dup := fun l => NoDup l;
        edge_no_dup := fun l => NoDup l;
        core_edge_disjoint := true;
        edge_weight_valid := fun f => snd f ∈ [0,1];
        edge_weight_normalized := sum (fun f => snd f) [("Commutative", 0.5)] ≤ 1;
      |} in
      {|
        ci_role := role;
        ci_rels := [
        ...
        ci_unique := fun obj' r' rels' => 
          core_function r' obj' = core_function role obj ∧
  ...
  | "Category_PreCategory" =>
      let role := {|
        ...
        core_edge_disjoint := true;
        edge_weight_valid := fun f => snd f ∈ [0,1];
        edge_weight_normalized := sum (fun f => snd f) [("Preserved_By_Equivalence", 0.6)] ≤ 1;
      |} in
      {|
        ci_role := role;
        ci_rels := [
  ...
  | "Geometry_SphereManifold" =>
      let role := {|
        ...
        core_function := fun x => (forall (p : R), Geometry.SphereRiemannCurvature x = p → p > 0);
        func_necessary := fun x => S.(prop_category) = SphereManifold;
        core_no_dup := fun l => NoDup l;
        edge_no_dup := fun l => NoDup l;
        core_edge_disjoint := true;
        edge_weight_valid := fun f => snd f ∈ [0,1];
        edge_weight_normalized := sum (fun f => snd f) [("Curvature_Dependent", 0.7)] ≤ 1;
      |} in
      {|
        ci_role := role;
        ci_rels := [
          {|
            ...
            related_objs := [obj; S.(id)];
            rel_rule := fun a b => Geometry.SphereRiemannCurvature a = Geometry.SphereRiemannCurvature b;
            rel_axiom_dep := exists ax : Axiom, In ax S.(axioms) ∧ (forall a b, rel_rule a b → ax);
          |}
        ];
        ci_unique := fun obj' r' rels' => 
...
Lemma core_feat_equiv : forall (S : FormalSystem) (r1 r2 : FunctionalRole S),
  ...
  (forall (f : string * R), In f r1.(edge_features) ↔ In f r2.(edge_features)) →
  r1 = r2.
Proof.
...
Proof.
  intros S r1 r2 H_id.
  apply DefinitiveRelation.extensionality.
  - exact H_id.
  - reflexivity.
...
Proof.
  intros S1 S2 map.
  apply S2.(ci_unique) (map_obj map S1.(id)) (cid_of S2 (map_obj map S1.(id))).
...
Theorem functional_role_determines_identity : forall (S : FormalSystem) (obj1 obj2 : S.(carrier)),
  core_function (ci_role (cid_of S obj1)) obj1 = core_function (ci_role (cid_of S obj2)) obj2 ∧
  Forall2 (fun r1 r2 => rel_rule r1 = rel_rule r2) (ci_rels (cid_of S obj1)) (ci_rels (cid_of S obj2)) →
  obj1 = obj2.
Proof.
  intros S obj1 obj2 [H_core H_rels].
  apply (cid_of S obj1).(ci_unique) obj2 (ci_role (cid_of S obj2)) (ci_rels (cid_of S obj2)); auto.
Qed.
...
Theorem relation_prior_to_object : forall (S : FormalSystem) (obj : S.(carrier)) (cid : ConceptIdentity S obj),
  ci_rels cid = [] → ¬(exists (obj' : S.(carrier)), core_function (ci_role cid) obj' = core_function (ci_role cid) obj).
Proof.
  intros S obj cid H_rels_empty H_exist.
  destruct H_exist as [obj' H_core].
  apply functional_role_determines_identity in H_core.
  apply H_rels_empty in H_core.
  contradiction.
Qed.

(* 定理3：身份具有系统相对性（FRF核心主张3，依赖跨系统映射） *)
Theorem identity_system_relativity : forall (S1 S2 : FormalSystem) (map : CrossSystemMapping S1 S2) (obj1 : S.(carrier)),
  core_function (ci_role (cid_of S1 obj1)) obj1 ↔ core_function (ci_role (cid_of S2 (map_obj map obj1))) (map_obj map obj1).
Proof.
  intros S1 S2 map obj1.
  split.
  - intro H_core.
    apply map_preserves_axioms map (core_function (ci_role (cid_of S1 obj1)) obj1).
    destruct H_core as [ax [H_ax_in H_ax_impl]].
    exists ax; split; [apply H_ax_in | apply H_ax_impl].
  - intro H_core.
    apply map_preserves_axioms map (core_function (ci_role (cid_of S2 (map_obj map obj1))) (map_obj map obj1)).
    destruct H_core as [ax [H_ax_in H_ax_impl]].
    exists ax; split; [apply H_ax_in | apply H_ax_impl].
Qed.
...
Proof.
  ...
  - split; [apply HinitZ | apply HinitZ'].
  - apply Forall2_nil.
Qed.
...
Theorem geometry_vacuum_relativity : forall (M1 M2 : SphereManifold) (map : CrossSystemMapping (SphereManifold_to_FormalSystem M1) (SphereManifold_to_FormalSystem M2)),
  core_function (ci_role (cid_of (SphereManifold_to_FormalSystem M1) M1)) M1 ↔
  core_function (ci_role (cid_of (SphereManifold_to_FormalSystem M2) (map_obj map M1))) (map_obj map M1).
Proof.
  intros M1 M2 map.
...
Qed.
Where SphereManifold_to_FormalSystem (M : SphereManifold) : FormalSystem := {|
  ...
  |};
  axioms := [SphereMetric_pos_def M; SphereRiemannCurvature M > 0];
...
Export cid_of core_feat_equiv rel_rule_consistent op_preserves_id map_preserves_id.
...
Export SphereManifold_to_FormalSystem.
